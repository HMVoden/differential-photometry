...
@composite
def three_lists(
    draw: DrawFn, elements: SearchStrategy, min_size: int = 0, max_size: int = 0
):
...
@given(
    three_lists(floats(allow_infinity=False)),
)
def test_conversion(data):
    lf, li, ls = data
    sts = StarTimeseries(time=lf, mag=li, error=ls)
    assert sts.error.dtype == np.float32
    assert sts.mag.dtype == np.float32
    assert sts.time.dtype == np.datetime64


@given(three_lists(text(alphabet=string.digits)))
