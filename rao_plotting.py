...
import rao_math as math
import rao_stats as stats
import rao_utilities as util


def plot_and_save_all(df: pd.DataFrame, plot_type='magnitude', uniform_y_axis=False, **saving):
    ...
    if plot_type == 'magnitude':
        ...
        raw_diff_magnitudes(star_frames=star_frames,
...
def raw_diff_magnitudes(star_frames, uniform_y_axis=False, **saving):
    if uniform_y_axis == True:
        ...
        logging.debug(
            "Maximum raw magnitude variation is: %s", mag_max_variation)
        logging.debug(
    ...
    with ProcessPoolExecutor(max_workers=4) as executor:
        if saving is not None:
            ...
            plot_function = partial(create_4x1_raw_diff_plot,
                                    mag_max_variation=mag_max_variation, diff_max_variation=diff_max_variation)
            saving_function = partial(
                save_plots, plot_function=plot_function, **saving)
...
def create_4x1_raw_diff_plot(star, mag_max_variation=None, diff_max_variation=None):
    day_frames = data.split_on_new_days(star)
    days = len(day_frames)
    star_name = star['name'].unique()[0]
    fig, axes = plt.subplots(nrows=4, ncols=days, figsize=(
        5*days, 15), sharex=False)
    if days == 1:
        axes = np.array(axes)
    ...
    fig.suptitle(
        ('Raw and differential magntitude of star ' + star_name))
    fig.autofmt_xdate()
    fig.tight_layout()
    return fig
