...
def calculate_differential_photometry(df: pd.DataFrame, varying: pd.DataFrame = None) -> pd.DataFrame:
    ...
    average_error = np.sqrt(
        np.sum(subtracted_err**2, axis=1)) / N

    average = util.arrange_for_dataframe(
        reference, average_diff_mags, average_error)
    average = {'average_diff_mags': average[0],
